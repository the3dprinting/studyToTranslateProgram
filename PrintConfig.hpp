...
template<> inline t_config_enum_values ConfigOptionEnum<InfillPattern>::get_enum_values() {  //静态函数，根据传入的InfillPattern返回组成的map
    t_config_enum_values keys_map;
...
template<> inline t_config_enum_values ConfigOptionEnum<SupportMaterialPattern>::get_enum_values() {  //静态函数，根据传入的SupportMaterialPattern返回组成的map
    t_config_enum_values keys_map;
...
template<> inline t_config_enum_values ConfigOptionEnum<SeamPosition>::get_enum_values() { //静态函数，根据传入的SeamPosition返回组成的map
    t_config_enum_values keys_map;
...
class PrintConfigDef   //打印配置定义
{
    public:
    static t_optiondef_map def;
    
    static t_optiondef_map build_def();  //这是最长的一个函数了！其实里面全是string→ConfigOptionDef的map，然后返回这个map……
};
...
class StaticPrintConfig : public virtual StaticConfig   //静态打印配置，设置所有打印默认参数！
{
    public:
    StaticPrintConfig() {
        this->def = &PrintConfigDef::def;
