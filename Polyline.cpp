...
Polyline::equally_spaced_points(double distance) const
{
    ...
    for (Points::const_iterator it = this->points.begin() + 1; it != this->points.end(); ++it) {
        ...
        double take = segment_length - (len - distance);  // how much we take of this segment 需要取得的这一段线段的长度
        Line segment(*(it-1), *it);
        points.push_back(segment.point_at(take));
        it--;
        len = -take;  //因为it--了，所以下一次的len的长度一开始就负了take长度的距离。
    }
    return points;
}
...
Polyline::simplify_by_visibility(const T &area)
{
    ...
    size_t s = 0;
    while (s < pp.size() && !area.contains(pp[s])) {  //注意：Expolygon类里面有成员函数contains！！
        ++s;                                           //虽然polygon也有contains，但是只有点，没有线段的重载函数，根据下面用到线段推测应该是Expolygon
    }
    ...
    for (size_t s = start; s < pp.size() && !pp.empty(); ++s) {
        ...
        for (size_t e = pp.size()-1; e > (s + 1); --e) {
            if (area.contains(Line(pp[s], pp[e]))) {
                ...
                // repeat recursively until no further simplification is possible
                return this->simplify_by_visibility(area);   //在这里return是因为如果已经删除了几个点，不能再进行for循环了，想要继续删除，必须用递归做ruturn值了。
            }
        }
    }
    */
...
Polyline::split_at(const Point &point, Polyline* p1, Polyline* p2) const
{
    ...
    for (Lines::const_iterator line = lines.begin(); line != lines.begin() + line_idx + 1; ++line) {
        if (!line->a.coincides_with(p)) p1->points.push_back(line->a);  //这里的if表明：如果point点到polyline的线段集合中最近的点刚好是polyline里的一个端点，则舍去了这个端点！
    }
