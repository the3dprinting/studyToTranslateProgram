...
{  
  ...
  ClipperPath_to_Slic3rMultiPoint(polynode.Contour, &(*expolygons)[cnt].contour);  //这里加上多边形的轮廓
  (*expolygons)[cnt].holes.resize(polynode.ChildCount());  
  for (int i = 0; i < polynode.ChildCount(); ++i)
  {
    ClipperPath_to_Slic3rMultiPoint(polynode.Childs[i]->Contour, &(*expolygons)[cnt].holes[i]);  //这里加上多边形可能出现的n个洞
...
void _clipper(ClipperLib::ClipType clipType, const Slic3r::Polygons &subject, 
    const Slic3r::Polygons &clip, Slic3r::Polylines* retval, bool safety_offset_)   //这个函数头文件，没有，因为只有上一个函数需要使用，暂时没看懂！
{
...
void safety_offset(ClipperLib::Paths* paths)    //头文件中没有的函数，表示将一个路径偏置10.0的距离。为啥会安全？！
{
    // scale input
