...
class PrintRegion  //代表拥有相同打印配置的一块区域，函数实现在PrintRegion.cpp里面
{
    friend class Print;
	...
	//上面的函数根据输出参数和config来得到新的Flow::new_from_config_width(...)并返回(推测是静态函数)
    private:
    Print* _print;
...
class PrintObject   //函数实现在PrintObject.cpp里面
{
    friend class Print;
...
class Print   //实现方法在Print.cpp里面
{
    public:
    PrintConfig config;    //打印配置
    PrintObjectConfig default_object_config;   //默认物体配置
    PrintRegionConfig default_region_config;   //默认区域配置
    PrintObjectPtrs objects;                   //打印物体指针
    PrintRegionPtrs regions;                   //打印区域指针
    ...
    void clear_objects();
    PrintObject* get_object(size_t idx);   //根据idx返回objects对应打印物体的指针
