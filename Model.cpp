...
Model::delete_material(t_model_material_id material_id)
{
    ...
    if (i != this->materials.end()) {
        delete i->second;    //materials的map的第二个元素是个ModelMaterial*指针，所有要先释放内存，再删除！
...
{
    while (!this->materials.empty())    //只要map非空，就一个一个删除之
...
ModelObject::delete_volume(size_t idx)
{
    ...
    delete *i;     //*i是一个ModelVolume*   因此要先delete清除内存
    this->volumes.erase(i);     //然后在删除其在vector中的地方
    this->invalidate_bounding_box();   
}
...
ModelObject::center_around_origin()
{
    ...
    Sizef3 size = bb.size();  //返回包围盒总长，宽，高组成的Sizef3，肯是一个点
    vector.x -= size.x/2;
    vector.y -= size.y/2;
    //这时vector就是物体移动到原点平台正上方的位移矢量
    this->translate(vector);
    this->origin_translation.translate(vector);   //更新origin_translation
    ...
    if (!this->instances.empty()) {
        for (ModelInstancePtrs::const_iterator i = this->instances.begin(); i != this->instances.end(); ++i) {
            ...
            v.rotate((*i)->rotation, (*i)->offset);  //以offset为中心逆时针旋转rotation度，rotation是弧度。
            v.scale((*i)->scaling_factor);           //坐标放大scaling_factor倍
            (*i)->offset.translate(v.x, v.y);      //核心步骤，实际就是更新了(*i)->offset！其他没做任何事情
        }
