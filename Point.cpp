...
Point::nearest_point_index(const Points &points) const
{
    ...
    for (Points::const_iterator it = points.begin(); it != points.end(); ++it)
        p.push_back(&*it);    //这里不理解，为什么不直接用it？
...
Point::distance_to(const Line &line) const
{
    ...
    //计算时设line上一点c满足(a→c):(c→b)=t:1-t(等式左边是矢量比，因为值肯是负的)，则c点坐标是(a.x+t*dx,a.y+t*dy)
    //this→c·a→b=0，解得t的值。很明显，t<0则线段上a到this点最短，t>0则是b到this点最短。
...
Point::perp_distance_to(const Line &line) const
{
    ...
    //this点到又ab组成的线的的垂直距离=向量a→this叉乘向量a→b再除以向量a→b的模。
